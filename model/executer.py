...
import platform
from webelement import WebBrowser
import threading
...
def launch_browser(url, PROXY_HOST, PROXY_PORT):
    ...
    if env.threadlocal.TESTING_BROWSER.upper() == 'FIREFOX':
        ...
        if platform.uname()[0] =='Windows':
            try:
                ...
                log.step_fail('chrome 启动失败！')
            finally:
                env.THREAD_LOCK.release()
        else:
            try:
                env.THREAD_LOCK.acquire()
                env.size = common.setDisplay()
                ...
                log.step_normal('linux chrome 启动成功！')
            except (WebDriverException, Exception) as e:
                log.step_fail('chrome 启动失败:[%s]' % e)
            finally:
                env.THREAD_LOCK.release()
        ...
        #DesiredCapabilities.PHANTOMJS['phantomjs.page.settings.userAgent'] = random.choice(env.USER_AGENTS)
        ...
        if platform.uname()[0] =='Windows':
            ...
            try:
                ...
                log.step_warning('phantoms 启动失败 [%s]' % e)
                try:
                    log.step_warning("try to start firefox again!")
                    time.sleep(20)
                    browser = webdriver.PhantomJS(env.PHANTOMJS, service_args=service_args, desired_capabilities=dcap, service_log_path =phantomjslog)
            ...
            try:
                ...
                log.step_warning('phantoms 启动失败 [%s]' % e)
                try:
                    log.step_warning("try to start phantomjs again!")
                    time.sleep(20)
                    browser = webdriver.PhantomJS('/usr/bin/phantomjs', service_args=service_args, desired_capabilities=dcap, service_log_path =phantomjslog)
                except:
                    log.handle_error()
    ...
    browser.set_page_load_timeout(60) #url超时时间
...
def quit_browser(browser):
    try:
        ...
        browser.close()
        browser.quit()
        if platform.uname()[0] =='Linux' and (env.threadlocal.TESTING_BROWSER.upper() == 'Firefox' or env.threadlocal.TESTING_BROWSER.upper() == 'chrome'):
            env.VDISPLAY.stop()
