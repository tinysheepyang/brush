...
import log, env ,MySQLdb
import json, urllib2, random, time
from fake_useragent import UserAgent
import random, platform, codecs, threading
...
def praserJsonFile():
    if len(env.p.lrange('proxy_list', 0, -1)) == 0:
        ...
        try:
            ...
            s = json.loads(data)
            if len(s['msg']) == 0:
                proxy = Check(random.choice(list(set(s['data']['proxy_list']))))
                return proxy
            else:
                time.sleep(30)
        ...
        if len(env.p.lrange('proxy_list', 0, -1)) > 2:
            log.step_normal('从redis删除一个ip！')
            return env.p.rpop('proxy_list')
        else:
            return random.choice(list(set(env.p.lrange('proxy_list', 0, -1))))
...
def Check(ip):
    try:
        ...
        if len(content) >= 1000:
            ...
            log.step_normal('出现验证码或IP被封杀 [%s]' % ip)
            praserJsonFile()
    except (URLError, HTTPError) as e:
        log.step_normal('代理ip无效 [%s]' % ip)
        praserJsonFile()
