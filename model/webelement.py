...
class WebElement:
    ...
    @compatiblemethod
    def Brush(cls, url, count):
        cls.count = count
        while cls.count < 10:
            log.step_normal(u"开始查找元素 [%s]:" % cls.__name__)

            cls.__wait()
            elements = env.threadlocal.BROWSER.find_elements(cls.by, cls.value)

            log.step_normal(u'已找到元素！')
            for ele in elements:
                try:
                    log.step_normal("Begin to match [%s] : [%s]" % (ele.text.split("/")[0].strip(), url))
                    if ele.text.split("/")[0].strip() == url or ele.text.split("/")[0].strip() == 'city.emao.com':
                        log.step_normal('已匹配到百度链接 [%s] 开始点击' % url)
                        flg, plist = cls.layer()
                        if flg:
                            ele.click()

                        log.step_normal('已点击百度链接')
                        env.CLICKBRAND = 1
                        cls.flag = True

                        log.step_normal('Switch to the new focus')
                        WebBrowser.SwitchToNewPopWindow()

                        log.step_normal('Operating a drop-down box')
                        WebBrowser.ScrollToPro()

                        log.step_normal('Random jump out')
                        try:
                            env.threadlocal.BROWSER.get(random.choice(env.JUMP_OUT))
                        except Exception,e:
                            log.step_normal('跳出url失败 [%s]' % e)
                        log.step_normal('Brush Done!')
                        break
                    else:
                        cls.flag = False
                except StaleElementReferenceException,e:
                    log.step_normal('element 失效 [%s]' % e)
                    break

            if cls.flag == True:
                break
            else:
                log.step_normal("The [%s] page not found the [%s], click next page" % (cls.count+1, url))
                flg, plist = cls.layer()
                if flg and len(plist) !=0:
                        plist[-1].click()
                        log.step_normal('next page')

                cls.count = cls.count + 1
                continue
        if cls.count == 10:
            log.step_normal('Has to turn over [%s] pages,not found [%s]' % (cls.count, url))
    ...
    def layer(cls):
        ...
        if j == 120:
            log.step_normal('点击链接或者下一页失败，该proxy有问题')
            sys.exit(1)
    ...
    def clicktarget(cls, target):
        ...
        if index > 5 and page == 1:
            ...
            common.data(env.IP, env.KEYWORD, env.TARGET, 0, "no_find", -1, -1)
            ...
            try:
                ...
                common.data(env.IP, env.KEYWORD, env.TARGET, 0, "time_out", -1, -1)
    ...
    def TypeIn(cls, value):
        ...
        env.BRAND = value
        env.BRAND = env.BRAND + '_' + 'xiala' + '_' + common.stamp_date()
